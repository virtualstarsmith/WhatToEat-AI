...
## Session 4: 推荐域规格化重构·③④⑤ 并行实现与合并

**Date**: 2026-06-24
**Task**: 推荐域规格化重构·③④⑤ 并行实现与合并
**Branch**: `feat/ai-recommend-experience`

### Summary

用 git worktree 真并行实现 ③④⑤（注：发现 Explore agent 在本平台部分只读，③ 落地、④⑤ 自行实现）。三次 merge 解决区域冲突：④ mystery.js 取③scenes+恢复红包入口（用户决策保留 mystery 红包 UI，用 coupon-float 组件）+ app.json 合并 plugins/usingComponents；⑤ index/mystery/test 三冲突取⑤版。集成验证 7/7：poi_id稳定、canonical/alias根治面馆≠面食(1.0795>0.8198)、首页打分、AI4层容错、冲突矩阵补全。回归11/11。spec 收口 4 条单一事实源约定。父任务归档。
