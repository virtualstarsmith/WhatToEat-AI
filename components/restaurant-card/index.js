...
// variant:
//   'index'  —— 首页：reason 带 💡 前缀，命中 shopEntry 时显示 🎫 按钮
//   'mystery'—— 盲盒：reason 不带前缀，附加 mystery-card/card-mismatch 类，无 🎫
Component({
  options: {
    addGlobalClass: true,
    multipleSlots: false
  ...
  methods: {
    ...
    onCopyAddr() {
      this.triggerEvent('copyaddr', { address: this.data.card.address });
    },
    onCoupon() {
